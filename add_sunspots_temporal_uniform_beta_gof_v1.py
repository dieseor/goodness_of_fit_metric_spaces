...
FILES = {
Path("real_data/sunspots/run_sunspots_cycle23_temporal_uniform_beta_gof.R"): r'''#!/usr/bin/env Rscript
...
Path("tests/testthat/test_sunspots_temporal_uniform_beta_gof.R"): r'''library(testthat)
...
def main():
    ...
    if not required.exists():
        fail(
            "The parsimonious temporal-model module is missing. "
            "Apply the first patch before this one."
        )
        ...
        if rscript:
            for path in FILES:
                run(
                    rscript,
                    "-e",
                    f"parse(file={str(path)!r})"
                )
        else:
            print(
                "Rscript was not found; R syntax parsing was skipped."
            )
    except Exception:
        for path in reversed(created):
            path.unlink(missing_ok=True)
        print(
            "Patch failed; newly created files were removed.",
            file=sys.stderr
        )
        raise

    print("Temporal uniform+Beta GOF files created successfully.")
