...
# for the cleaned Risoe 125 m November-December data, 1996-2003.
#
# The four sampling patterns are strict arithmetic progressions with step 4:
# start1 = 1,5,...,29; start2 = 2,6,...,30;
# start3 = 3,7,...,27; start4 = 4,8,...,28.
...
make_nov_dec_configs <- function() {
  lapply(names(strict_step4_patterns), function(pattern_id) {
    list(
      dataset_id = sprintf("risoe_clean_125m_nov_dec_%s_1996_2003", pattern_id),
      window = "nov_dec",
      months = c(11L, 12L),
      pattern = pattern_id,
...
run_nov_dec_fast_ks_cvm <- function(
    B = 5000L,
    n_cores = 12L,
    base_seed = 2026071200L,
    input_nc = file.path("real_data", "wind", "risoe_m_all.nc"),
    output_dir = file.path(
      "real_data", "wind", "month_diagnostics",
      sprintf("nov_dec_125m_ks_cvm_b%d", as.integer(B)),
  ...
  output_csv <- file.path(
    output_dir,
    sprintf("risoe_125m_nov_dec_fast_sampleks_ks_cvm_b%d.csv", B)
  )
...
if (sys.nframe() == 0L) {
  ...
  results <- run_nov_dec_fast_ks_cvm(B = B, n_cores = n_cores)
