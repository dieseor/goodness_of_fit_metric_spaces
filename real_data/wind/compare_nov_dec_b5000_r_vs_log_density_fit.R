...
# the strict-step-four November-December B=5000 samples, with and without the
# log-speed transformation.

source(file.path(
  "real_data", "wind",
  "plot_risoe_125m_nov_dec_b5000_r_cylinder_density_contours.R"
))

r_summary_path <- file.path(
  results_dir, "r_cylinder_density_contours",
  "risoe_125m_r_cylinder_density_contours_summary.csv"
)
log_summary_path <- file.path(
  results_dir, "log_cylinder_density_contours",
  "risoe_125m_log_cylinder_density_contours_summary.csv"
)
output_path <- file.path(results_dir, "nov_dec_b5000_r_vs_log_density_fit_comparison.csv")
...
compare_nov_dec_r_vs_log <- function(n_theta = 241L, n_linear = 401L) {
  ...
  if (nrow(summaries) != 4L) {
    stop("Expected four matched starts in the r/log summary CSV files.", call. = FALSE)
  }
  ...
  rows <- lapply(seq_len(nrow(summaries)), function(i) {
    ...
    if (nrow(case) != row$n_r || nrow(case) != row$n_log) {
      stop("Sample-size mismatch in density comparison.", call. = FALSE)
    }
...
if (sys.nframe() == 0L) {
  result <- compare_nov_dec_r_vs_log()
