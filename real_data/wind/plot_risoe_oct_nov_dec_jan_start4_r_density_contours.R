...
# r = speed / mean(speed), for Risoe 125 m, Oct--Nov--Dec--Jan, start4.
# The sample matches the 1996--2004 screening: 125 m observations before
# 2004-11-12 only; no missing observations are imputed.
...
# Reuse the validated HvMF, reflected-KDE, HDR, and contour machinery.
source(file.path(repo_root, "real_data", "wind", "plot_risoe_125m_nov_dec_b5000_r_cylinder_density_contours.R"))
...
build_target_case <- function(selected) {
  ...
  data.frame(
    ...
    month_group = factor(ifelse(rows$month == 10L, "October (added month)", "November--January"),
                         levels = c("November--January", "October (added month)")),
    x0 = cosh(r), x1 = sinh(r) * cos(theta),
...
plot_target_case <- function(case, output_dir) {
  ...
  p <- ggplot2::ggplot(grid_data, ggplot2::aes(theta_deg, r)) +
    ...
    ggplot2::geom_point(data = case, ggplot2::aes(theta_deg, r, colour = month_group), inherit.aes = FALSE, shape = 16, size = 1.35, alpha = .86) +
    ...
    ggplot2::scale_colour_manual(values = c("November--January" = "#4D4D4D", "October (added month)" = "#C0392B"), name = "Observation month") +
    ...
    ggplot2::labs(
      ...
      title = "Risø 125 m wind data on the unwrapped cylinder",
  ...
  stem <- file.path(output_dir, "risoe_125m_oct_nov_dec_jan_start4_r_density_contours")
  ggplot2::ggsave(paste0(stem, ".pdf"), p, width = 10.5, height = 4.3, units = "in")
  ggplot2::ggsave(paste0(stem, ".png"), p, width = 10.5, height = 4.3, units = "in", dpi = 320)
  data.frame(
...
run_oct_nov_dec_jan_start4_r_density_contours <- function(
    output_dir = file.path(repo_root, "real_data", "wind", "month_diagnostics", "oct_nov_dec_jan_125m_start4_r_density_contours_1996_2004")) {
  ...
  utils::write.csv(summary, file.path(output_dir, "risoe_125m_oct_nov_dec_jan_start4_r_density_contours_summary.csv"), row.names = FALSE)
  summary
}

if (sys.nframe() == 0L) print(run_oct_nov_dec_jan_start4_r_density_contours(), row.names = FALSE, digits = 6)
