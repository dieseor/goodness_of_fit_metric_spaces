...
# Log-cylinder density plots for the strict-step-four Risoe 77 m
# November-December B=5000 analysis. Both estimates are densities with
# respect to dtheta dz; the fitted HvMF therefore includes sinh(exp(z))exp(z).

source(file.path("real_data", "wind", "plot_risoe_cylinder_density_contours.R"))

plot_height_m <- 77L
plot_speed_col <- "ws77"
plot_direction_col <- "wd77"
plot_clean_before <- as.Date("2007-08-01")

results_dir <- file.path(
  repo_root, "real_data", "wind", "month_diagnostics",
  "nov_dec_77m_ks_cvm_b5000", "fast_sampleks_strict_step4"
)
results_csv <- file.path(
  results_dir, "risoe_77m_nov_dec_fast_sampleks_ks_cvm_b5000.csv"
)
if (!file.exists(results_csv)) {
  stop("B=5000 77 m November-December results CSV not found: ", results_csv, call. = FALSE)
}

screening_results <- utils::read.csv(results_csv, stringsAsFactors = FALSE)
screening_results <- screening_results[
  screening_results$window == "nov_dec" & screening_results$status == "ok", , drop = FALSE
]
...
run_nov_dec_77m_b5000_log_density_plots <- function(
...
if (sys.nframe() == 0L) {
  ...
  result <- run_nov_dec_77m_b5000_log_density_plots(patterns = requested_patterns)
