...
sunspots_joint_draw_parametric_sphere_panel <- function(
    lon_seq,
    lat_seq,
    parametric_density,
    parametric_thresholds,
...
sunspots_joint_draw_contour_legend <- function(
  ...
  graphics::text(0.60, 0.50, "Nonparametric", adj = c(0, 0.5), cex = 0.82)
  graphics::points(
...
sunspots_joint_draw_method_legend <- function() {
  graphics::plot.new()
  graphics::plot.window(xlim = c(0, 1), ylim = c(0, 1))
  graphics::legend(
    "center",
...
sunspots_joint_render_contour_sphere_grid <- function(
  ...
  sunspots_joint_draw_method_legend()
# graphics::mtext(
...
sunspots_joint_draw_inline_method_legend <- function(
    x = -1.04,
    y = 1.04,
    cex = 0.52) {
  graphics::legend(
...
sunspots_joint_render_individual_contour_sphere_pdfs <- function(
    lon_seq,
    lat_seq,
    per_window,
    output_dir,
    ...
    pdf_width = 3.2,
    pdf_height = 3.2) {
  if (length(per_window) < 1L) {
    stop("`per_window` must contain at least one window.")
  }
  if (length(hdr_colors) != length(hdr_levels)) {
    stop("`hdr_colors` and `hdr_levels` must have the same length.")
  }
  ...
  if (length(pdf_width) != 1L ||
      length(pdf_height) != 1L ||
      !is.finite(pdf_width) ||
      !is.finite(pdf_height) ||
      pdf_width <= 0 ||
      pdf_height <= 0) {
    stop("`pdf_width` and `pdf_height` must be positive finite scalars.")
  }

  dir.create(output_dir, recursive = TRUE, showWarnings = FALSE)
  output_paths <- character(length(per_window))

  for (i in seq_along(per_window)) {
    output_paths[[i]] <- file.path(
      output_dir,
      sprintf("cycle23_joint_spatial_window_%02d_contours.pdf", i)
    )
    ...
    tryCatch(
      {
        ...
        sunspots_joint_draw_parametric_sphere_panel(
          lon_seq = lon_seq,
          lat_seq = lat_seq,
          parametric_density = per_window[[i]]$parametric,
          parametric_thresholds = per_window[[i]]$hdr_parametric$threshold,
...
run_sunspots_cycle23_joint_spatial_window_kde_plots <- function(
  ...
  individual_pdf_dir <- file.path(
    output_dir,
    "individual_spheres_pdf"
  )
  individual_pdf_paths <- sunspots_joint_render_individual_contour_sphere_pdfs(
    lon_seq = lon_seq,
    lat_seq = lat_seq,
    per_window = per_window,
    output_dir = individual_pdf_dir,
    hdr_levels = hdr_levels,
    back_alpha = back_alpha
  )
  ...
  invisible(list(
    ...
    plot_path = plot_path,
    individual_pdf_paths = individual_pdf_paths,
  ...
  for (arg in args) {
    if (arg %in% c("--help", "-h")) {
      cat(paste0(
        ...
        "--hdr_levels=0.5,0.8,0.95 --back_alpha=NUMBER --bandwidth_multiplier=NUMBER\n"
      ))
