...
timestamp_tag_beta_overnight <- function() {
  format(Sys.time(), "%Y%m%d_%H%M%S")
}

expected_raw_rows_beta_overnight <- function(M_outer,
                                             n_values,
                                             statistics) {
  as.integer(M_outer) * length(as.integer(n_values)) * length(statistics)
}

expected_summary_rows_beta_overnight <- function(n_values,
                                                 statistics,
                                                 alphas) {
  length(as.integer(n_values)) * length(statistics) * length(as.numeric(alphas))
}

beta_overnight_output_complete <- function(output_dir,
                                           expected_raw_rows,
...
run_beta_composite_stage <- function(output_dir,
  ...
  expected_raw <- expected_raw_rows_beta_overnight(
    M_outer = M_outer,
    n_values = n_values,
    statistics = statistics
  )
  expected_summary <- expected_summary_rows_beta_overnight(
    n_values = n_values,
    statistics = statistics,
    alphas = alphas
  )

  if (beta_overnight_output_complete(
...
run_beta_comets_stage <- function(output_dir,
  ...
  run_comets_rotational_mixtures_short_long(
    ...
    models = c("rotational_beta_mixture2"),
    B = as.integer(B),
    statistics = statistics,
...
run_beta_overnight_20260602 <- function(
  output_root = file.path("output", "overnight_beta_20260602"),
  max_beta_hours = 10,
  M_outer = 500L,
  n_cores_outer = 12L,
  comet_B = 1000L,
