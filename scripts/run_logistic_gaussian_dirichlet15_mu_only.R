...
# Dedicated runner for the Logistic-Gaussian / Dirichlet(1.5,...,1.5) scenario.
#
...
lg_dirichlet15_mu_betas <- c(0, 0.25, 0.5, 1)
lg_dirichlet15_mu_scenarios <- c("dirichlet15")

lg_dirichlet15_mu_seed <- function(base_seed, design_id, replication, stream) {
  as.integer((as.numeric(base_seed) + 1000003 * design_id +
    1009 * replication + 10000019 * stream) %% 2147483647) + 1L
}

dirichlet15_alpha <- function(d) {
  d <- as.integer(d)
...
r_dirichlet15 <- function(n, d) {
  if (!requireNamespace("gtools", quietly = TRUE)) {
    stop(
      "Package 'gtools' is required for the Dirichlet(1.5,...,1.5) scenario."
    )
  }
  gtools::rdirichlet(
    n = as.integer(n),
    alpha = dirichlet15_alpha(d)
  )
}

generate_lg_dirichlet15_mu_sample <- function(scenario, n, d, beta,
                                            nu = 4,
                                            t_standardized = TRUE,
                                            dirichlet_concentration_multiplier = 1) {
  scenario <- match.arg(scenario, lg_dirichlet15_mu_scenarios)
  n <- as.integer(n)
  d <- as.integer(d)
...
lg_dirichlet15_mu_design <- function(dimensions, n_values, beta_values, scenarios, M) {
  ...
  if (!all(beta_values %in% lg_dirichlet15_mu_betas)) {
    stop("beta_values must be a subset of c(0, 0.25, 0.5, 1).")
  }
  if (!all(scenarios %in% lg_dirichlet15_mu_scenarios)) {
    stop("scenarios must be a subset of c('dirichlet15').")
  }
  ...
  catalog <- expand.grid(
    scenario = lg_dirichlet15_mu_scenarios,
    d = dimensions,
    n = n_values,
    beta = lg_dirichlet15_mu_betas,
    stringsAsFactors = FALSE
  )
...
lg_dirichlet15_mu_key <- function(x) {
  paste(
    x$scenario,
    x$d,
    x$n,
...
run_logistic_gaussian_dirichlet15_mu_only <- function(
    output_dir,
    M = 100L,
    B = 499L,
    dimensions = c(2L, 5L),
  ...
  if ("dirichlet15" %in% scenarios && !requireNamespace("gtools", quietly = TRUE)) {
    stop(paste(
  ...
  manifest <- transform(
    ...
    alternative_component = "Dirichlet(1.5,...,1.5)",
  ...
  lock <- file.path(output_dir, ".logistic_gaussian_dirichlet15_mu_only.lock")
  ...
  if (!all(names(empty_results()) %in% names(results))) {
    ...
    lg_dirichlet15_mu_key(results[conforming(results, derivative_mc_size), , drop = FALSE])
  } else {
    character()
  }
  pending <- design[!lg_dirichlet15_mu_key(design) %in% done, , drop = FALSE]
  started <- Sys.time()

  run_one <- function(row) {
    began <- proc.time()[["elapsed"]]
    data_seed <- lg_dirichlet15_mu_seed(base_seed, row$design_id, row$replication, 0L)
    bootstrap_seed <- lg_dirichlet15_mu_seed(base_seed, row$design_id, row$replication, 1L)
    derivative_seed <- lg_dirichlet15_mu_seed(base_seed, row$design_id, row$replication, 2L)
    ...
    tryCatch({
      set.seed(data_seed)
      x <- generate_lg_dirichlet15_mu_sample(
      ...
      fit <- withCallingHandlers(
        if (identical(base$scenario, "dirichlet15")) {
      ...
      if (!conforming(out, derivative_mc_size)) {
        ...
        out$error_message <- "Requested Logistic-Gaussian Dirichlet(1.5) mu-only fast configuration was not effective."
      }
      out
    }, error = function(e) {
      base$status <- "error"
...
if (sys.nframe() == 0L) {
  ...
  output_dir <- cli$output_dir %||%
    file.path("simulation_results", "logistic_gaussian_dirichlet15_mu_only")

  run_logistic_gaussian_dirichlet15_mu_only(
    ...
    scenarios = parse_character_csv(cli$scenario, c("dirichlet15")),
