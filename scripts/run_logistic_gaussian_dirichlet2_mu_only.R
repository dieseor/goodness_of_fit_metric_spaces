...
# Dedicated runner for the Logistic-Gaussian / Dirichlet(2,...,2) scenario.
#
# Fitted composite null:
#   ilr(X) ~ N_d(mu, I_d), with only mu estimated.
#
# Data-generating model:
#   P_beta = (1-beta) LG(0, I_d)
#            + beta Dirichlet(2,...,2),
# where the Dirichlet distribution has d+1 components.
#
# This runner deliberately uses the existing Logistic-Gaussian mu-only
# bootstrap implementation. It does not modify the AR(1) t4 scenario,
# the sigma-shape adapter, or any production runner.
...
lg_dirichlet2_mu_betas <- c(0, 0.25, 0.5, 1)
lg_dirichlet2_mu_scenarios <- c("dirichlet2")

lg_dirichlet2_mu_seed <- function(base_seed, design_id, replication, stream) {
  as.integer((as.numeric(base_seed) + 1000003 * design_id +
    1009 * replication + 10000019 * stream) %% 2147483647) + 1L
}

dirichlet2_alpha <- function(d) {
  d <- as.integer(d)
...
r_dirichlet2 <- function(n, d) {
  if (!requireNamespace("gtools", quietly = TRUE)) {
    stop(
      "Package 'gtools' is required for the Dirichlet(2,...,2) scenario."
    )
  }
  gtools::rdirichlet(
    n = as.integer(n),
    alpha = dirichlet2_alpha(d)
  )
}

generate_lg_dirichlet2_mu_sample <- function(scenario, n, d, beta,
                                            nu = 4,
                                            t_standardized = TRUE,
                                            dirichlet_concentration_multiplier = 1) {
  scenario <- match.arg(scenario, lg_dirichlet2_mu_scenarios)
  n <- as.integer(n)
  d <- as.integer(d)
...
lg_dirichlet2_mu_design <- function(dimensions, n_values, beta_values, scenarios, M) {
  ...
  if (!all(beta_values %in% lg_dirichlet2_mu_betas)) {
    stop("beta_values must be a subset of c(0, 0.25, 0.5, 1).")
  }
  if (!all(scenarios %in% lg_dirichlet2_mu_scenarios)) {
    stop("scenarios must be a subset of c('dirichlet2').")
  }
  ...
  catalog <- expand.grid(
    scenario = lg_dirichlet2_mu_scenarios,
    d = dimensions,
    n = n_values,
    beta = lg_dirichlet2_mu_betas,
    stringsAsFactors = FALSE
  )
...
lg_dirichlet2_mu_key <- function(x) {
  paste(
    x$scenario,
    x$d,
    x$n,
...
conforming <- function(x, derivative_mc_size) {
  expected_vhat_method <- "fitted_gaussian_influence_reparameterization"
  x$status == "ok" &
...
run_logistic_gaussian_dirichlet2_mu_only <- function(
    output_dir,
    M = 100L,
    B = 499L,
    dimensions = c(2L, 5L),
  ...
  if ("dirichlet2" %in% scenarios && !requireNamespace("gtools", quietly = TRUE)) {
    stop(paste(
  ...
  design <- lg_dirichlet2_mu_design(dimensions, n_values, beta_values, scenarios, M)
  ...
  manifest <- transform(
    ...
    null_model = "LG(mu,I_d)",
    estimated_parameters = "mu",
    data_generating_null = "LG(0,I_d)",
    alternative_component = "Dirichlet(2,...,2)",
  ...
  lock <- file.path(output_dir, ".logistic_gaussian_dirichlet2_mu_only.lock")
  ...
  if (!all(names(empty_results()) %in% names(results))) {
    ...
    lg_dirichlet2_mu_key(results[conforming(results, derivative_mc_size), , drop = FALSE])
  } else {
    character()
  }
  pending <- design[!lg_dirichlet2_mu_key(design) %in% done, , drop = FALSE]
  started <- Sys.time()

  run_one <- function(row) {
    began <- proc.time()[["elapsed"]]
    data_seed <- lg_dirichlet2_mu_seed(base_seed, row$design_id, row$replication, 0L)
    bootstrap_seed <- lg_dirichlet2_mu_seed(base_seed, row$design_id, row$replication, 1L)
    derivative_seed <- lg_dirichlet2_mu_seed(base_seed, row$design_id, row$replication, 2L)
    ...
    tryCatch({
      set.seed(data_seed)
      x <- generate_lg_dirichlet2_mu_sample(
      ...
      fit <- withCallingHandlers(
        if (identical(base$scenario, "dirichlet2")) {
      ...
      if (!conforming(out, derivative_mc_size)) {
        ...
        out$error_message <- "Requested Logistic-Gaussian Dirichlet(2) mu-only fast configuration was not effective."
      }
      out
    }, error = function(e) {
      base$status <- "error"
...
if (sys.nframe() == 0L) {
  ...
  output_dir <- cli$output_dir %||%
    file.path("simulation_results", "logistic_gaussian_dirichlet2_mu_only")

  run_logistic_gaussian_dirichlet2_mu_only(
    ...
    scenarios = parse_character_csv(cli$scenario, c("dirichlet2")),
