...
simulate_limit_supremum_ecdf <- function(M, F_t) {
  cov_matrix <- outer(F_t, F_t, function(a, b) pmin(a, b) - a * b)
  simulate_limit_gaussian(cov_matrix, M = M, seed = NULL)
}
...
run_case <- function(case_name, n, M, t_grid, r_sampler, p_cdf, out_file) {
  cat("===", case_name, "===\n")
  ...
  make_hist_plot(
    ...
    title_txt = paste0(case_name, ": Histograms only (grid=2, no seed)"),
...
# 1) Uniform(0,1) with t-grid of 2 points
t_grid_uniform <- c(0.33, 0.67)
out_uniform <- "output/convergence/gaussian_process/normal/prueba_ecdf_uniform_0_1_n10_M10000_hist_grid2_noseed.png"
...
# 2) Normal(mu=3, var=0.01 => sd=0.1) with t-grid of 2 points
mu <- 3
sd <- 0.1
u_grid <- c(0.33, 0.67)
t_grid_normal <- qnorm(u_grid, mean = mu, sd = sd)
out_normal <- "output/convergence/gaussian_process/normal/prueba_ecdf_normal_mu3_var001_n10_M10000_hist_grid2_noseed.png"
