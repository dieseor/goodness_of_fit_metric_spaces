resolve_small_circle_calibration_path <- function(...) {
  candidates <- c(
    file.path(...),
    file.path("..", ...),
    file.path("..", "..", ...)
...
calibration_study_script_small_circle <- resolve_small_circle_calibration_path(
  "bootstrap",
  "calibration_study.R"
)
source(calibration_study_script_small_circle)
...
build_small_circle_calibration_control <- function(L_max = 200L,
                                                   quad_n = 500L,
                                                   profile_method = "legendre") {
  list(
    small_circle_L_max = as.integer(L_max),
    small_circle_quad_n = as.integer(quad_n),
    small_circle_profile_method = as.character(profile_method)
  )
}
...
run_small_circle_calibration_sequential <- function(
  output_root = file.path(
    "output",
    "bootstrap_calibration",
    "small_circle_simple_composite_M500_B500_bootlocal"
  ),
  ...
  common_control <- build_small_circle_calibration_control(
    L_max = small_circle_L_max,
    quad_n = small_circle_quad_n,
    profile_method = small_circle_profile_method
  )

  simple_scenarios <- default_small_circle_simple_calibration_scenarios(
  ...
  composite_scenarios <- default_small_circle_composite_calibration_scenarios(
    kappa_values = kappa_values,
    nu_values = nu_values
  )
  ...
  run_rows_df <- do.call(rbind, run_rows)
  consolidated <- consolidate_sequential_outputs(
    run_rows = run_rows_df,
    consolidated_dir = file.path(output_root, "_consolidated")
  )

  list(
    output_root = output_root,
    manifest = run_rows_df,
    consolidated = consolidated
  )
}

if (sys.nframe() == 0L) {
  run_small_circle_calibration_sequential()
}